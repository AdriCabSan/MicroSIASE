...
(private val context: Context) {
    ...
    /**
     * method to check InputEditText has valid matricula .
     *
     * @param textInputEditText
     * @param textInputLayout
     * @param message
     * @return
     */
    fun isInputEditTextMatricula(textInputEditText: TextInputEditText, textInputLayout: TextInputLayout, message: String): Boolean {
        val value = textInputEditText.text.toString().trim()

        var numeric = value.matches("[0-9]*".toRegex())

        if (value.isEmpty() || !numeric) {
            textInputLayout.error = message
            hideKeyboardFrom(textInputEditText)
            return false
        } else {
